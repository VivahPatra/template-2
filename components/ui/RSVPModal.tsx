...
export default function RSVPModal({ open, onClose, onSend, defaultMessage, brideName, groomName }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" style={{ background: 'rgba(13,5,8,0.85)', backdropFilter: 'blur(8px)' }} onClick={onClose}>
      <div
        className="relative w-full max-w-md rounded-2xl p-6"
        style={{ background: 'var(--color-bg)', border: '1px solid var(--color-accent)', boxShadow: '0 0 60px var(--color-accent-dim), 0 20px 60px rgba(0,0,0,0.6)' }}
      ...
      >
        ...
        <div className="flex items-center justify-center gap-4 mb-6">
          <button onClick={() => setGuestCount(c => Math.max(1, c - 1))}
            className="w-10 h-10 rounded-full font-sans text-lg" style={{ background: 'var(--color-surface2)', color: 'var(--color-accent)', border: '1px solid var(--color-border)' }}>−</button>
