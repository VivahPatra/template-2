    ...
    >
      <div className="relative" style={{ width: circleSize, height: circleSize }}>
        ...
        <div
          ...
          style={{ boxShadow: `0 0 55px 20px ${color}55` }}
        />
      ...
      <div className="text-center mt-3">
        <EditableText field="name" index={index} arrayField="events" tag="p" className="font-display tracking-wide glow-text" style={{ color: 'var(--color-text)', fontSize: isHero ? '1.25rem' : '1rem' }}>
          {event.name}
        </EditableText>
        <p
        ...
        >
          <EditableText field="date" index={index} arrayField="events">{event.date.split(',')[0]}</EditableText> · <EditableText field="time" index={index} arrayField="events">{event.time}</EditableText>
        </p>
      </div>
      ...
      >
        <EditableText field="venue" index={index} arrayField="events" tag="p" className="font-serif text-sm" style={{ color: 'var(--color-text)', opacity: 0.85 }}>
          {event.venue}
        </EditableText>
        <EditableText field="venueAddress" index={index} arrayField="events" tag="p" className="font-sans text-xs mt-1" style={{ color: 'var(--color-muted)', opacity: 0.7 }}>
        ...
        <a
          ...
          className="inline-block mt-2 font-sans text-xs tracking-wider underline underline-offset-2 hover:opacity-100 transition-opacity duration-200"
      ...
      <div className="max-w-5xl mx-auto px-6 py-28 relative">
        ...
        >
          ...
          <motion.div variants={fadeUp} className="flex justify-center mt-6">
            <svg viewBox="0 0 320 14" width="320" height="14" aria-hidden>
              <path d="M10,7 L310,7" fill="none" stroke="var(--color-accent)" strokeWidth="1.5" opacity="0.38"/>
              <path d="M10,11 L310,11" fill="none" stroke="var(--color-accent)" strokeWidth="0.6" opacity="0.2"/>
              <rect x="75"  y="4" width="6" height="6" fill="var(--color-accent2)" opacity="0.52" transform="rotate(45 78 7)"/>
              <rect x="155" y="3" width="8" height="8" fill="var(--color-accent)"  opacity="0.65" transform="rotate(45 159 7)"/>
              <rect x="235" y="4" width="6" height="6" fill="var(--color-accent2)" opacity="0.52" transform="rotate(45 238 7)"/>
            </svg>
          </motion.div>
        </motion.div>
        ...
        <div className="relative">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10 md:gap-10 md:relative md:z-10">
            {row1.map((ev, i) => (
              <EventNode key={ev.id} event={ev} index={i} delay={i * 0.1} />
            ))}
          </div>
        ...
        <div className="flex justify-center mt-14" aria-hidden>
          <svg viewBox="0 0 600 24" width="400" height="24">
            <path d="M0,12 Q150,0 300,12 Q450,24 600,12" fill="none" stroke="var(--color-accent)" strokeWidth="0.8" opacity="0.2"/>
